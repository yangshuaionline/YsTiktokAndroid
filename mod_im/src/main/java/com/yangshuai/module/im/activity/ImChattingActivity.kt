...
import com.yangshuai.module.im.databinding.ImActChattingBinding
...
/**
 * @Author yangshuai
 * @Date 2023-05-09 周二 13:35
 * @Description
 * 一对一单聊
 */
@SuppressLint("ClickableViewAccessibility")
@Route(path = RouterPath.IM.IM_CHATTING)
class ImChattingActivity : BaseToolBarActivity<ImActChattingBinding, ImGroupViewModel>() {
    var screenHeight: Int? = 0
    var keyHeight: Int? = 0
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.im_act_chatting)
    }
    ...
    private fun initView() {
        ...
        connectRong()//连接融云
        mBinding.btSubmitImChatting.setOnClickListener {
            sendMSG()
        }
        // 获取屏幕高度
        ...
        mBinding.etContentImChatting.addOnLayoutChangeListener { _, _, _, _, bottom, _, _, _, oldBottom -> //old是改变前的左上右下坐标点值，没有old的是改变后的左上右下坐标点值
...
//                Log.i("=======", "软键盘关闭")
    ...
    override fun initParam() {
        ...
        RouterManager.addPage(RouterPath.IM.IM_CHATTING,this)
    }
        ...
        if(token == saveToken){
            ...
            RongIMClient.connect(token,object : RongIMClient.ConnectCallback(){
                override fun onSuccess(t: String?) {
                    ...
                    //发送消息
                    mBinding.btSubmitImChatting.setOnTouchListener { v, event ->
                        when (event?.action) {
