...
public class SimpleGridView extends RecyclerView {
    ...
    // item布局
    private int itemLayoutId = R.layout.base_grid_item;

    public SimpleGridView(@NonNull Context context) {
        super(context, null);
    }

    public SimpleGridView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SimpleGridView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SimpleGridView, defStyle, 0);
        int itemSpacing = (int) a.getDimension(R.styleable.SimpleGridView_itemSpacing, dp2Px(context, 16));
        spanCount = a.getInteger(R.styleable.SimpleGridView_spanCount, 3);
        itemLayoutId = a.getResourceId(R.styleable.SimpleGridView_itemLayout, R.layout.base_grid_item);
        a.recycle();
        ...
        adapter = new Adapter(itemLayoutId) {
            @Override
                ...
                itemView.setOnClickListener(v -> {
                    if (selectedItems.contains(item)) {
                        ...
                        if (onItemSelectListener != null)
                            onItemSelectListener.onSelect(item, false);
                    } else {
                        if (!muiltSelected) {
                            selectedItems.clear();
                        }
                        ...
                        if (onItemSelectListener != null)
                            onItemSelectListener.onSelect(item, true);
                    }
